...
def read_transform_plink_files(geno_path):
    ...
    #fam_data = pd.read_csv(full_fam_name,sep=r"\s+",header=None)
    ...
    if ped_2_field.shape[1] == len(marker_list):
        ...
        if ped_2_field.dtype == "int64":
            ...
            # for each individual x and each marker i, count the refernence allele number from the ref_allele[i]
            for i, col in enumerate(ped_2_field.columns):
                target_char = ref_allele[i]
        ...
        if ref_allele.dtype == "int64":
            print("Variant allele is integer, no need to transform.")
        else:
            for i in range(len(marker_list)):
                ped_2_field_reshaped[:,i,:] = np.where(ped_2_field_reshaped[:,i,:] == ref_allele[i], 0, 1)
    ...
    ped_2_field_merged = pd.concat([ped_1_field,ped_2_field],axis=1)
