...
class Sugarcane:
    ...
    def select(self,field,sample_size=0):
        ...
        if field is tuple or field is list:
            ...
            csvname = PATH  + "_".join(field) + "_" + str(sample_size) + ".csv"
    ...
    def select_single(self,label,name,trait,sample_size=0):
        ...
        if label and name and trait:
            sub_pheno = self.phenotypes[self.phenotypes[field] == name][["Clone","Region",trait]]
            merge = pd.merge(sub_pheno,self.genotypes,left_on="Clone",right_on="sample")
            print(merge.shape)
            merge.dropna(axis=0,how="any",inplace=True)

            if sample_size != "all":
                dataset = merge.sample(int(sample_size))
            else:
                dataset = merge
            dataset.drop(["sample","Clone"],axis=1,inplace=True)
            csvname = PATH + "_".join([str(name),trait]) + "_" + str(sample_size) + ".csv"
            dataset.to_csv(csvname, sep="\t")
...
def main():
    ...
    req_grp.add_argument('-r', '--region', type=bool, help="add regions (T/F)", default=False)
